...
const SequenciasCRUD: React.FC<SequenciasCRUDProps> = ({ empresaId }) => {
  ...
  useEffect(() => {
    console.log('SequenciasCRUD - empresaId:', empresaId);
    if (empresaId) {
      loadData();
    } else {
      console.log('SequenciasCRUD - empresaId não definido, aguardando...');
    }
  }, [empresaId]);

  useEffect(() => {
    console.log('SequenciasCRUD - estado sequencias mudou:', sequencias);
  }, [sequencias]);

  useEffect(() => {
    console.log('SequenciasCRUD - estado loading mudou:', loading);
  }, [loading]);

  const loadData = async () => {
    console.log('SequenciasCRUD - loadData chamado, empresaId:', empresaId);
    setLoading(true);
    try {
      console.log('SequenciasCRUD - fazendo requisição para API...');
      const data = await sequenciaService.getAll({ empresa_id: empresaId });
      console.log('SequenciasCRUD - dados recebidos:', data);
      setSequencias(data);
    } catch (error) {
              ...
              <>
                ...
                {sequencias.length === 0 && (
                  <div className="empty-state">
                    Nenhuma sequência cadastrada para esta empresa (ID: {empresaId})
                  </div>
                )}
              </>
            )}
